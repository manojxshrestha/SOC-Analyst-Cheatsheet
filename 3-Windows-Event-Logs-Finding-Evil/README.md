...
To quickly identify the available logs, we can leverage the **-ListLog** parameter:
...
This command provides us with valuable information about each log, including:
...
To explore event log providers, use the **-ListProvider** parameter:
...
**Retrieving events from WinRM operational log:**
...
**Retrieving oldest events first (use -Oldest parameter):**
...
Use **-Path** parameter to read exported .evtx files:
...
Use **-FilterHashtable** parameter to filter events efficiently.
...
> ⚠️ **NOTE**: If we observe Sysmon event IDs 1 and 3 (related to "dangerous" or uncommon binaries) occurring within a short time frame, it could potentially indicate the presence of a process communicating with a Command and Control (C2) server.
...
> ⚠️ **NOTE**: The above will filter between the start date inclusive and the end date exclusive.

### Filtering Events with FilterXml

Use **-FilterXml** parameter for complex XML queries:
...
Use **-FilterXPath** parameter for XPath queries:
...
> 💡 **KEY POINT**: `Properties[21]` corresponds to the **ParentCommandLine** field in Sysmon Event ID 1. This helps detect encoded PowerShell commands!
...
> 💡 **KEY DIFFERENCE**: Sysmon captures **parent process** and **file hashes** - crucial for malware detection!
...
> ⚠️ **IOCs to watch**: calc.exe from Desktop, WININET.dll from user directory, unsigned DLLs in signed processes
...
Monitor for:
- **clr.dll** loading in processes that shouldn't run .NET (e.g., spoolsv.exe)
- **clrjit.dll** in unusual processes
- Any non-PowerShell processes becoming "managed" (.NET processes)

> 🔍 **Detection**: These DLLs indicate .NET runtime loading in processes that don't typically use it!
...
> 🚨 **ALERT**: Multiple failed attempts + lsass access = likely credential dumping!
...
```cmd
# Install Sysmon with hash logging
...
> 📌 **Popular configs**: [SwiftOnSecurity](https://github.com/SwiftOnSecurity/sysmon-config) | [Olaf Hartong](https://github.com/olafhartong/sysmon-modular)
...
> 🚨 **CRITICAL**: This is a **major indicator** - attackers often clear logs to hide their tracks!
...
> 🔍 **RED FLAG**: Service accounts with RDP = likely compromise!
...
> 📌 **KEY ADVANTAGE**: ETW provides **kernel-level visibility** that Sysmon cannot see!
...
> 💡 **USE CASE**: Always use `-ets` when working with ETW sessions!
- Real-time event capture
- Highly customizable filtering
