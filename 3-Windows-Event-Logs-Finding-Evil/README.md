...
Event log providers serve as the sources of events within the logs. This command provides us with an overview of the available providers and their associations with specific logs.
...
This example retrieves the first 50 events from the System log.
...
**Retrieving oldest events first:**
...
If you have an exported .evtx file from another computer or you have backed up an existing log, you can utilize the Get-WinEvent cmdlet to read and query those logs:
...
Consider an intrusion detection scenario where a suspicious network connection to a particular IP (52.113.194.132) has been identified:
...
This script will retrieve all Sysmon network connection events (ID 3), parse the XML data for each event, and filter the results to include only events where the destination IP matches the suspected IP.

### Detecting .NET Assembly Loading

In the "Tapping Into ETW" section we were looking for anomalous clr.dll and mscoree.dll loading activity. The command below leverages Sysmon's Event ID 7:
...
**Find Sysinternals tool installation:**
...
The -Property * parameter selects all properties of the event:
